...
class SubmitAssignmentScreenModel(
        ...
        screenModelScope.launch {
            ...
            assignmentRepository.getAttachments(userId,assignmentId).collect{result->
                when(result){
                    ...
                    is Result.SuccessWithData -> {
                        _uiState.value = _uiState.value.copy(oldSubmission = result.data,isLoading = false)
                    }
                    else->{}
                }
    ...
    fun submitAssignment(assignmentId : String,userId : String,attachments: List<AssignmentAttachment>){
        screenModelScope.launch {
    ...
    fun handleAction(action: SubmitAssignmentUiAction) {
        when (action) {
            is SubmitAssignmentUiAction.OnUploadAttachment -> {
                _uiState.update {
                    it.copy(newSubmission  = it.newSubmission + listOf(action.attach))
                }
                with(uiState.value){
                    submitAssignment(
                        assignmentId = assignment.id,
                        userId = user.id,
                        attachments = newSubmission
                    )
                }
            }
            else->{}
